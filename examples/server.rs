...
async fn handle_connection(connection: Connection) {
    ...
    while let Some(msg) = connection.rx.next().await {
        println!("Received a message from {}: {}", connection.addr, msg);
        connection.tx.unbounded_send(msg).expect("Failed to forward message");
    }
}
...
async fn accept_connection(stream: TcpStream) {
    ...
    while let Some(message) = ws_stream.next().await {
        ...
        msg_tx.unbounded_send(message).expect("Failed to forward request");
